...
Result=json.dumps({  
      ...
      "/home Use space"     : os.popen("df -h | grep /run$ | awk '{print $(NF-1)}'" + XARGS).read(),
      "/home Use space"     : os.popen("df -h | grep /run$ | awk '{print $(NF-1)}'" + XARGS).read(),
      "/home Total space"   : os.popen("df -h | grep /run$ | awk '{print $(NF-4)}'" + XARGS).read(),
      "/Data Use space"     : os.popen("df -h | grep /run$ | awk '{print $(NF-1)}'" + XARGS).read(),
      "/Data Total space"   : os.popen("df -h | grep /run$ | awk '{print $(NF-4)}'" + XARGS).read(),
