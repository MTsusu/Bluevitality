...
#工程目录名（第1参数）,ps工程名和在此目录下查找JAR包用
PROJECT_NAME="iscadapi-core"
#JAR包名（第2参数）及节点数
APPLICATION="logger-admin-service-impl-1.0.0-SNAPSHOT.jar"
NODE_NUMBER=4
#启停脚本路径
START_APP_SCRIPT_PATH="$HOME/sbin/start-${PROJECT_NAME}.sh"
STOP_APP_SCRIPT_PATH="$HOME/sbin/stop-${PROJECT_NAME}.sh"

#若未定义则使用传参：脚本 工程名 JAR包名 启动脚本路径 停止脚本路径
[ -z $PROJECT_PATH ] && PROJECT_NAME=$1                     #工程名称
[ -z $APPLICATION ] && APPLICATION=$2                       #JAR包名称
[ -z $NODE_NUMBER ] && NODE_NUMBER=$3                       #JAR包节点数           
[ -z $START_APP_SCRIPT_PATH ] && START_APP_SCRIPT_PATH=$4   #启动脚本路径
[ -z $STOP_APP_SCRIPT_PATH ] && STOP_APP_SCRIPT_PATH=$5     #停止脚本路径
...
function START_STOP() {
    ...
    sh $HOME/sbin/Node_stop.sh >> $HOME/sbin/node_pullup.log
    sleep 2
