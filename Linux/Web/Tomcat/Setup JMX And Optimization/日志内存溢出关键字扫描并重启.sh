...
for log in $LOGS
{
    ECPCORE_NEW_NUM=`echo $log | grep -oP '.*(?=/logs)' | awk -F'/' '{print $NF}'`    #从日志路径截取出节点名部分    #替换了旧的：| awk -F"[_/]" '{print $7"_"$8}'
    [[ -f $COUNT_LOG_PATH/${ECPCORE_NEW_NUM}.log-${DATE} ]] || echo 1 > $COUNT_LOG_PATH/${ECPCORE_NEW_NUM}.log-${DATE} 
    
    LINE_NUM=`cat $COUNT_LOG_PATH/${ECPCORE_NEW_NUM}.log-${DATE}`   #取出扫描的上次结束行位置
    ALL_LINE=`wc -l $log | awk '{print $1}'`                        #取出日志文件总行数
    
    echo $log
    echo $ALL_LINE
    GC_COUNT=`sed -n "$LINE_NUM,${ALL_LINE}p" $log |grep -sc "${GC_ERROR}"`   #从特定日志行范围内扫描内存溢出关键字的次数
    echo $ALL_LINE > $COUNT_LOG_PATH/${ECPCORE_NEW_NUM}.log-${DATE}         #记录本次执行扫描时结束的行数
    
    if [ $GC_COUNT -gt 0 ]; then
        echo "restart $ECPCORE_NEW_NUM"
        ECPCORE_NEW_PID=`ps aux | grep $ECPCORE_NEW_NUM | egrep -v "grep|tail" | awk '{print$2}'`
        $HOME/jdk7/bin/jmap -dump:format=b,file=$COUNT_LOG_PATH/${ECPCORE_NEW_NUM}_$(date "+%F_%H%M%S")_hprof  $ECPCORE_NEW_PID
        server_stop $ECPCORE_NEW_NUM
        server_start $ECPCORE_NEW_NUM
    fi
} 
