#### 设置 Token
![jenkins-img](资料/curl/01.png)
#### 使用 curl 进行构建的触发操作
![jenkins-img](资料/curl/02.png)
...
curl -s -X POST -u "${USERNAME_PASSWORD}" "${JOB_ADDRESS}/build" --data-urlencode json="{'parameter': [{'name':\"${TAG_NAME}\",'value':\"${TAG_VALUE##*/}\"}]}" && sleep 1
#获取上次成功构建的编号
